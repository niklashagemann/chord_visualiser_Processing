      ...
      for (int c=0; c<c_bar[b]; c++) {
        if (structured[b][c] != null) {
          ...
          if (music.fifths(prev_temp_index, temp_index)) {
            ...
            noFill();
            strokeWeight(arrow_thickness);//strokeWeight(3);
            ...
            float x0 = prev_x_pos + 4 + (factor_b/factor)*x_offset;
            float y0 = prev_y_pos - 5 + (factor_b/factor)*y_offset;
