...
class Song {
  ...
  void load(int song_no) {
    song = realbook.songname[song_no];
    String file_name = song + ".txt";
    ...
    for (int b=0; b<indexed.length; b++) {
      for (int c=0; c<c_bar[b]; c++) {
        if (structured[b][c] != null) {
          ...
          color bubble_color = color(220);
          int bubble_width = 30;

          if (dominant == true) {
            bubble_color = color(247, 217, 120);
          }
          if (minor == true) {
            bubble_color = transitions_color;
          }
          if (minor == true && dominant == true) {
            bubble_color = color(140, 160, 190);
          }
          if (major == true) {
            bubble_color = color(255, 125, 80, 140);
          }
          if (diminished == true || augmented == true) {
            bubble_color = color(255, 155, 150);
          }
          ...
          make_bubble(bubble_color, bubble_width, x_pos, y_pos, 0, bubble_x_offset, bubble_y_offset); // last input variable is the mode (0 = circle, 1 = rectangle)
          ...
          if (diminished == true) {
            extra_x = 6;
            textSize(14);
            text("º", x_pos + 5 + 0.4*extra_x, y_pos -3);
          }
          if (augmented == true) {
            extra_x = 6;
            textSize(14);
  ...
  void make_bubble(color temp_color, int temp_width, float temp_x, float temp_y, int temp_mode, int bubble_x_offset, int bubble_y_offset) {
    noStroke();
    if (temp_mode == 0) {
      fill(temp_color);
      //ellipseMode(CENTER);
